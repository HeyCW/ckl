...
class PrintHandler:
    ...
    def _generate_excel_invoice_optimized(self, container, barang_list, container_id):
        ...
        try:
            ...
            ws.merge_cells(f'A{current_row}:L{current_row}')  # Changed from F to L to match main table width
            ...
            for cost_name, cost_value in costs_surabaya.items():
                ...
                ws[f'M{current_row}'] = cost_value  # Changed from G to M (column 13)
            ...
            ws[f'A{current_row}'] = ""
            ws[f'M{current_row}'] = total_biaya_surabaya  # Changed from G to M
            ...
            ws.merge_cells(f'A{current_row}:L{current_row}')  # Changed from F to L
            ...
            for cost_name, cost_value in costs_samarinda.items():
                ...
                ws[f'M{current_row}'] = cost_value  # Changed from G to M
                ws[f'M{current_row}'].font = profit_font
                ws[f'M{current_row}'].alignment = right_align
                ws[f'M{current_row}'].number_format = '#,##0'
                current_row += 1
            ...
            ws[f'A{current_row}'] = ""
            ws[f'M{current_row}'] = total_biaya_samarinda  # Changed from G to M
            ws[f'M{current_row}'].font = Font(name='Arial', size=8, bold=True)
            ws[f'M{current_row}'].alignment = right_align
            ws[f'M{current_row}'].number_format = '#,##0'
            ws[f'M{current_row}'].border = thin_border
            ...
            ws[f'M{current_row}'] = profit_lcl  # Changed from G to M
